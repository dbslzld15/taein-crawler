...
from taein_crawler.crawler import TaeinCrawler
...
def init_runner(context: Context, run_by: str) -> typing.Callable:
    ...
    def runner() -> None:
        crawler = TaeinCrawler(context.config)
        crawler.run(run_by)
...
def cli(ctx: typing.Any) -> None:
    ctx.obj = {}
    config = taein_crawler.config.load()
